...
sub GSL_HISTOGRAM_PDF_SAMPLE : Tests {
    my $H = gsl_histogram_alloc(100);
    gsl_histogram_set_ranges_uniform($H, 0, 100);
    gsl_histogram_reset($H);

    # NOTE: shift by 1 so we avoid cumulative distribution to become identically zero.
    #  The cumulative dist is generated by gsl_histogram_pdf_init() below.
    #  If the cumulative dist was identically zero, gsl_histogram_pdf_sample() below
    #  would crash.
    gsl_histogram_shift($H, 1);
    my $p = gsl_histogram_pdf_alloc(100);
    gsl_histogram_pdf_init ($p, $H);
    ok_similar(gsl_histogram_pdf_sample( $p, 0.5 ), 50.0, "gsl_histogram_pdf_sample");
}
Test::Class->runtests;
