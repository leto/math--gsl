...
sub GSL_FIT_LINEAR : Tests {
 ...
 my (@x, @w, @y, $i);
   for ($i = 0; $i < 36; $i++) 
    ...
    my @got = gsl_fit_linear(@x, $xstride, @y, $ystride, 36);
    is($got[0], 0);
    ok(is_similar_relative($got[1], -0.262323073774029, 1e-10));
    ok(is_similar_relative($got[2], 1.00211681802045, 1e-10));
    ok(is_similar_relative($got[3], 0.232818234301152**2.0, 1e-10));
    ok(is_similar_relative($got[4], -7.74327536339570e-05, 1e-10));
    ok(is_similar_relative($got[5], 0.429796848199937E-03**2, 1e-10));
    ok(is_similar_relative($got[6], 26.6173985294224, 1e-10));
}
