...
=item gsl_ran_dirichlet_pdf($alpha, $theta)
...
=item gsl_ran_dirichlet_lnpdf($alpha, $theta)
...
=item gsl_ran_multinomial($r, $N, $P) 
...
=item gsl_ran_poisson($r, $mu)

This function returns a random integer from the Poisson distribution with mean $mu. $r is a gsl_rng structure. The probability distribution for Poisson variates is, p(k) = {$mu**$k \ $k!} exp(-$mu) for $k >= 0. $r is a gsl_rng structure

=item gsl_ran_poisson_pdf($k, $mu)
...
=item gsl_ran_weibull($r, $a, $b)

This function returns a random variate from the Weibull distribution. The distribution function is, p(x) dx = {b \over a^b} x^{b-1}  \exp(-(x/a)^b) dx for x >= 0. $r is a gsl_rng structure

=item gsl_ran_weibull_pdf($x, $a, $b)
